/*
Copyright 2017 The nholuongut Authors.
...
func makeUnions(extensions map[string]interface{}) ([]schema.Union, error) {
	...
	if iunions, ok := extensions["x-nholuongut-unions"]; ok {
		unions, ok := iunions.([]interface{})
		if !ok {
			return nil, fmt.Errorf(`"x-nholuongut-unions" should be a list, got %#v`, unions)
		}
		for _, iunion := range unions {
			union, ok := iunion.(map[interface{}]interface{})
			if !ok {
				return nil, fmt.Errorf(`"x-nholuongut-unions" items should be a map of string to unions, got %#v`, iunion)
			}
			unionMap := map[string]interface{}{}
			for k, v := range union {
				key, ok := k.(string)
				if !ok {
					return nil, fmt.Errorf(`"x-nholuongut-unions" has non-string key: %#v`, k)
				}
				unionMap[key] = v
			}
...
func (c *convert) VisitKind(k *proto.Kind) {
	preserveUnknownFields := c.preserveUnknownFields
	if p, ok := k.GetExtensions()["x-nholuongut-preserve-unknown-fields"]; ok && p == true {
	...
	ext := k.GetExtensions()
	if val, ok := ext["x-nholuongut-map-type"]; ok {
		switch val {
		case "atomic":
...
func (c *convert) VisitArray(a *proto.Array) {
	...
	if val, ok := ext["x-nholuongut-list-type"]; ok {
		if val == "atomic" {
			...
			l.ElementRelationship = schema.Associative
			if keys, ok := ext["x-nholuongut-list-map-keys"]; ok {
	...
	} else if val, ok := ext["x-nholuongut-patch-strategy"]; ok {
		if val == "merge" || val == "merge,retainKeys" {
			l.ElementRelationship = schema.Associative
			if key, ok := ext["x-nholuongut-patch-merge-key"]; ok {
