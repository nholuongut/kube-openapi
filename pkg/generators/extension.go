...
const extensionPrefix = "x-nholuongut-"
...
var tagToExtension = map[string]extensionAttributes{
	...
	"listMapKey": {
		xName:        "x-nholuongut-list-map-keys",
		kind:         types.Slice,
		enforceArray: true,
	},
	"listType": {
...
type extension struct {
	...
	xName  string   // Example: x-nholuongut-list-type
...
func parseExtensions(comments []string) ([]extension, []error) {
	...
	errors := []error{}
	// First, generate extensions from "+k8s:openapi-gen=x-nholuongut-*" annotations.
	values := getOpenAPITagValue(comments)
	for _, val := range values {
		// Example: x-nholuongut-member-tag:member_test
		if strings.HasPrefix(val, extensionPrefix) {
			...
			e := extension{
				...
				xName:  parts[0],           // Example: x-nholuongut-member-tag
	...
	for _, idlTag := range sortedMapKeys(tagValues) {
		...
		e := extension{
			...
			xName:  xAttrs.xName, // x-nholuongut-list-type
