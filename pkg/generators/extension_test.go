/*
Copyright 2018 The nholuongut Authors.
...
func TestSingleTagExtension(t *testing.T) {
	...
	var tests = []struct {
		...
		},
		{
			...
			comments: []string{
				"+k8s:openapi-gen=x-nholuongut-no-value",
				"+k8s:openapi-gen=x-nholuongut-member-success:success",
				"+k8s:openapi-gen=x-nholuongut-wrong-separator;error",
			},
			extensionTag:    "k8s:openapi-gen",
			extensionName:   "x-nholuongut-member-success",
...
func TestExtensionParseErrors(t *testing.T) {
	...
	var tests = []struct {
	...
	}{
		{
			...
			errorMessage: "x-nholuongut-no-value",
		},
		{
			...
			errorMessage: "x-nholuongut-wrong-separator;error",
		},
	}
...
func TestExtensionAllowedValues(t *testing.T) {
	...
	var successTests = []struct {
	...
	}{
		{
			e: extension{
				idlTag: "patchStrategy",
				xName:  "x-nholuongut-patch-strategy",
				values: []string{"merge"},
			},
		},
		{
		...
		},
		{
			e: extension{
				idlTag: "patchMergeKey",
				xName:  "x-nholuongut-patch-merge-key",
				values: []string{"key1"},
			},
		},
		{
...
func TestValidateMemberExtensions(t *testing.T) {
	...
	listMapKeysExtension := extension{
		idlTag: "listMapKey",
		xName:  "x-nholuongut-map-keys",
		values: []string{"key1"},
	}
	genExtension := extension{
		idlTag: "k8s:openapi-gen",
		xName:  "x-nholuongut-member-type",
